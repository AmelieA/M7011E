...
function createTables() {
    pg.connect(dbURL, function(err, client, done) {
		...
		client.query("CREATE TABLE Comments(location VARCHAR(255), text VARCHAR(255), login VARCHAR(255))");
		client.query("CREATE TABLE Images(location VARCHAR(255) REFERENCES Locations(location), img_name VARCHAR(255), login VARCHAR(255))", function() {
			done();
		});
	})
}
